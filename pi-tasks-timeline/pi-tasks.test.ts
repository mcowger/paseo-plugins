...
import { hasActivePiTasks } from "./shared/pi-tasks";
...
describe("Pi task timeline example", () => {
      ...
      items: [
        {
          ...
          data: {
            tasks: [
              { id: "1", text: "alpha task", status: "completed" },
              { id: "2", text: "beta task", status: "in_progress" },
              { id: "3", text: "gamma task", status: "pending" },
            ],
          },
        },
      ],
  ...
  it("maps Pi's example todo extension", () => {
    ...
    expect(result?.items[0]?.data).toEqual({
      tasks: [
        { id: "1", text: "alpha task", status: "completed" },
        { id: "2", text: "beta task", status: "pending" },
      ],
    });
  });

  it("identifies whether a snapshot has active tasks", () => {
    expect(
      hasActivePiTasks([
    ...
    expect(hasActivePiTasks([{ id: "1", text: "done", status: "completed" }])).toBe(false);
  });
