...
import type { PiSessionStats } from "../shared/rpc-types.js";
...
function toProviderUsage(stats: PiSessionStats): ProviderUsage | undefined {
  ...
  const contextWindowUsedTokens =
    typeof stats.contextUsage?.tokens === "number" ? stats.contextUsage.tokens : undefined;
...
export class PiUsagePoller {
  ...
  async completeTurn(turnId?: string): Promise<void> {
    ...
    try {
      usage = toProviderUsage(await this.options.readStats());
    } catch {
      return;
    }
  ...
  private publishUsage(usage: ProviderUsage | undefined, turnId?: string): void {
