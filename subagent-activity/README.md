...
This Paseo `v0.9.0` plugin adds an agent-scoped activity pane for monitoring managed Paseo
...
Provider-native activity is intentionally labeled separately because Paseo 0.9.0 does not expose a stable plugin-facing native-child registry. Native rows therefore do not claim model, token, or independent lifecycle data that the timeline does not provide.
...
The plugin targets Paseo `v0.9.0`.
